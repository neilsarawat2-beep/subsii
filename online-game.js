...
// ============ COPIED FROM WORKING 2-PLAYER MODE ============
function showMain() {
  ...
  // Show turn indicator if not my turn
  if(turn !== playerRole && players[turn]) {
    document.getElementById('ticker').innerText = `${players[turn].n.toUpperCase()}'S TURN...`;
  }
}

// CRITICAL FIX: Using EXACT working logic from 2-player mode
function showSub(m) {
  ...
  if(m==='ATTACK') {
    // EXACT copy from working 2-player mode
...
function handleAction() {
  ...
  // CRITICAL FIX: Exact working logic from 2-player mode
  if(curM.isItem) {
    ...
    // Attack logic (exact from 2-player mode)
...
function checkDeath() {
  Object.values(players).forEach(p => {
    if(p.cur <= 0 && p.items.BKUP > 0) {
      ...
      const t = document.getElementById('ticker');
      t.innerHTML = `<div class='crit-text' style='color:#0f0'>${p.revived === 2 ? "IS STILL THERE" : "REBOOTING"}</div>`;
      update();
      const stateUpdate = {};
      Object.keys(players).forEach(key => stateUpdate[key] = serializePlayer(players[key]));
      gameStateRef.update(stateUpdate);
...
function endTurn() {
  ...
  document.querySelectorAll('.online-sprite').forEach(s=>s.classList.remove('shake'));
