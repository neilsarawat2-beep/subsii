...
function hostRoom() {
  ...
  roomRef.child('players').on('value',(snap)=>{
    ...
    if(ps && Object.keys(ps).length>=needed){
      document.getElementById('waiting').style.display='none';
      startSel();
    } else if(ps){
      document.getElementById('waiting').innerText=`WAITING... (${Object.keys(ps).length}/${needed})`;
    }
  });
}
...
function joinRoom() {
  ...
  roomRef.once('value',(snap)=>{
    ...
    if(cnt>=needed){alert('ROOM FULL'); return;}
    ...
    roomRef.child('players').on('value',(snap)=>{
      const ps=snap.val();
      if(ps && Object.keys(ps).length>=needed) startSel();
      else if(ps) document.getElementById('waiting').innerText=`WAITING... (${Object.keys(ps).length}/${needed})`;
    });
  });
}
...
function checkReady(ps) {
  ...
  const needed=mode==='1v1'?2:4;
  if(list.length<needed) return;
  if(list.every(p=>p.ready) && !started){
    started=true;
    initBattle(ps);
  }
}
...
function initBattle(ps) {
  ...
  Object.keys(ps).forEach(key=>{
    ...
    players[key]={
      ...
      trainer:pd.trainer,
      trId:trainers[pd.trainer].id,
      trColor:trainers[pd.trainer].c,
      role:key,
      team:pd.team
    };
...
function setupUI() {
  ...
  [...teamA,...teamB].forEach(p=>{
    hud.innerHTML+=`
      <div style="flex:1; min-width:240px; padding:8px; border:1px solid #333; background:#0a0a0a; margin:5px; border-left:3px solid ${p.team==='A'?'#0f0':'#f00'};" id="hud-${p.role}">
...
function deserialize(data,orig) {
  if(!data||!orig) return orig;
  return {...orig,cur:data.cur,stun:data.stun,bleed:data.bleed,weak:data.weak,shield:data.shield,shieldLock:data.shieldLock,buff:data.buff,accMod:data.accMod,evasion:data.evasion,used:data.used,lastMoveName:data.lastMoveName,revived:data.revived,hasDealtDmg:data.hasDealtDmg,items:data.items};
}

function sync(state) {
  if(!state) return;
  Object.keys(players).forEach(k=>{if(state[k]) players[k]=deserialize(state[k],players[k]);});
  turn=state.turn;
...
function update() {
  Object.values(players).forEach(p=>{
    ...
    if(bar){
      bar.style.width=(Math.max(0,p.cur)/p.hp*100)+"%";
      bar.style.setProperty('--b-color',p.color);
    }
    ...
    if(st){
      let arr=[];
      if(p.bleed>0) arr.push(`BLEED(${p.bleed})`);
      if(p.weak>0) arr.push(`WEAK(${p.weak})`);
      if(p.buff>0) arr.push("BUFFED");
      if(p.evasion>0) arr.push("EVA");
      st.innerText=arr.join(" | ");
    }
...
function showSub(m) {
  ...
  const opps=Object.values(players).filter(p=>p.team!==a.team && p.cur>0);
  const o=opps[0];
  
  if(m==='ATTACK'){
    // EXACT working logic from 2-player mode
    ...
    // EXACT working logic from 2-player mode
    const itms=[
      {n:"Dudu",t:"HP",v:a.items.HP,d:"+7 HP"},
      {n:"ANTI",t:"ANTI",v:a.items.ANTI,d:"Clear status"},
      {n:"BUFF",t:"BUFF",v:a.items.BUFF,d:"+3 DMG"},
      {n:"BACKUP",t:"BKUP",v:a.items.BKUP,d:"Revive"}
    ];
    itms.forEach((it,i)=>{
      let b=document.getElementById('b'+i);
      b.style.display='block';
      b.innerText=`${it.n}(${it.v})`;
      b.disabled=(it.v<=0||(o&&(o.trId==="NAYSHA"||o.trId==="KESHAV")&&(it.t==="HP"||it.t==="ANTI")));
...
function handleAction() {
  ...
  const opps=Object.values(players).filter(p=>p.team!==att.team && p.cur>0);
  const def=opps[Math.floor(Math.random()*opps.length)];
  const t=document.getElementById('ticker');
  const sDef=def?document.getElementById(`sp-${def.role}`):null;
  
  document.getElementById('exec-trigger').style.display='none';
  
  let msg="";
  
  // CRITICAL: EXACT working item logic from 2-player mode
  if(curM.isItem){
    ...
    if(Math.random()*100>hit||dodged){
      t.innerHTML=dodged?"<div class='crit-text' style='color:#ff4500;'>[ DODGED ]</div>":"<div class='crit-text' style='color:#888;'>[ MISSED ]</div>";
      att.accMod=0;
    } else {
      let d=(curM.val||0);
      ...
      if(curM.t==="STUN"||curM.t==="SILENCE") def.stun=(curM.n==="Bald Shine"||curM.n==="Pause"||curM.n==="Totalitarian"||curM.n==="Out Syllabus"||curM.n==="Go out")?2:1;
      ...
      if(def.shield>0&&(d>0||curM.t==="WRATH")){
        if(att.buff>0){
          def.shield=0;
          def.shieldLock=true;
          msg="<div class='crit-text' style='color:#ff00ea;'>[ BROKEN ]</div>";
        } else if(curM.t==="WRATH"||curM.t==="RISK"){
          def.shield=0;
          msg="<div class='crit-text' style='color:#ff00ea;'>[ PIERCED ]</div>";
        } else {
          d=0;
          def.shield--;
          msg="<div class='crit-text' style='color:#00f3ff;'>[ BLOCKED ]</div>";
        }
      }
      ...
      if(d>0&&sDef){sDef.classList.add('shake'); att.hasDealtDmg=true;}
  ...
  const upd={'action/processed':true};
  Object.keys(players).forEach(k=>upd[k]=serialize(players[k]));
  stateRef.update(upd);
...
function checkDeath() {
  Object.values(players).forEach(p=>{
    if(p.cur<=0&&p.items.BKUP>0){
      ...
      document.getElementById('ticker').innerHTML=`<div class='crit-text' style='color:#0f0'>${p.revived===2?"STILL THERE":"REBOOTING"}</div>`;
      update();
      const upd={};
      Object.keys(players).forEach(k=>upd[k]=serialize(players[k]));
      stateRef.update(upd);
  ...
  const teamA=Object.values(players).filter(p=>p.team==='A'&&p.cur>0);
  const teamB=Object.values(players).filter(p=>p.team==='B'&&p.cur>0);
...
function endTurn() {
  ...
  let attempts=0;
  while(players[turnOrder[nextIdx]].cur<=0&&attempts<turnOrder.length){
...
window.addEventListener('beforeunload',()=>{
  if(roomRef) roomRef.child('players/'+myRole+'/connected').set(false);
});
